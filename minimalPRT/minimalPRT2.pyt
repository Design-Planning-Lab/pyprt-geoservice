...
SCRIPT_PATH = "C:\\Users\\SEED110\\Documents\\geoService\\minimalPRT\\minimalPRT2.pyt"
...
class Minimal:
    ...
    def execute(self, parameters, messages):
        ...
        if len(parameters) == 0 or parameters[0] != "cmd":
            import subprocess
            ab = subprocess.run(f"python {SCRIPT_PATH}",
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE
                                )
            arcpy.AddMessage(str(ab.stdout))
            arcpy.AddMessage(str(ab.stderr))
